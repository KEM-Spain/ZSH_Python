...
sel_list () {
	...
	[[ $(( BOX_X_COORD-XPAD )) -ge 1 ]] && SX=$(( BOX_X_COORD-XPAD )) || SX=${BOX_X_COORD}
	[[ $(( BOX_Y_COORD-YPAD )) -ge 1 ]] && SX=$(( BOX_X_COORD-YPAD )) || SY=${BOX_Y_COORD}
	SW=$(( YPAD * 2 + SW ))
	...
	if [[ ${BOUNDARY_SET} == 'false' ]];then
		...
		# Outer box w/ title
		[[ -n ${_SEL_LIST_HDR} ]] && tcup $((SX-1)) $(coord_center $(( _MAX_COLS )) ${#_SEL_LIST_HDR});echo -n ${BOLD}${(C)_SEL_LIST_HDR}${RESET}

		msg_unicode_box ${SX} ${SY} ${SW} ${SH} ${OUTER_BOX_COLOR} # OUTER box
		box_coords_set OUTER_BOX X ${SX} Y ${SY} W ${SW} H ${SH}

		CLEAN_TEXT=$(msg_nomarkup ${TITLE})
		tcup $(( SX+1 )) $(( SY+(SW/2)-(${#CLEAN_TEXT}/2) ));echo $(msg_markup ${TITLE})
		...
		if [[ -n ${_PAGE_OPTION_KEY_HELP} ]];then
			...
			tcup ${OPT_KEY_ROW} $(( SY+(SW/2)-(${#CLEAN_TEXT}/2) ));echo $(msg_markup ${_PAGE_OPTION_KEY_HELP})
		fi
