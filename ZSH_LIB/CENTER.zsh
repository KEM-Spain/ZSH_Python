...
get_box_center () {
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: OBJECT DIMS - HEIGHT:${WHITE_FG}${HEIGHT}${RESET} WIDTH:${WHITE_FG}${WIDTH}${RESET} X_OFF:${WHITE_FG}${X_OFF}${RESET} Y_OFF:${WHITE_FG}${Y_OFF}${RESET}"

	X=$(get_vert_center ${HEIGHT})
	Y=$(get_horz_center ${WIDTH})
	
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: Centers - Vertical X:${WHITE_FG}${X}${RESET}, Horizontal Y:${WHITE_FG}${Y}${RESET}"

	[[ ${X_OFF} -ne 0 ]] && X=$(( X + X_OFF ))
	[[ ${Y_OFF} -ne 0 ]] && Y=$(( Y + Y_OFF ))

	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: Applied offsets - X:${WHITE_FG}${X}${RESET}  Y:${WHITE_FG}${Y}${RESET}"
...
get_vert_center () {
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${_SCRIPT:t}->${0}:" "$(dbg_arglist "${@}")"
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: HEIGHT:${WHITE_FG}${HEIGHT}${RESET} REGION:${WHITE_FG}${REGION}${RESET}"
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: HEIGHT_CENTER:${WHITE_FG}${HEIGHT_CENTER}${RESET} REGION_CENTER:${WHITE_FG}${REGION_CENTER}${RESET}"
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: CENTERS AFTER ROUNDING - HEIGHT_CENTER:${WHITE_FG}${HEIGHT_CENTER}${RESET} REGION_CENTER:${WHITE_FG}${REGION_CENTER}${RESET}"

	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: Return value (${WHITE_FG}${REGION_CENTER}${RESET} - ${WHITE_FG}${HEIGHT_CENTER}${RESET}):$(( REGION_CENTER - HEIGHT_CENTER ))"
...
get_horz_center () {
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${_SCRIPT:t}->${0}:" "$(dbg_arglist "${@}")"
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: WIDTH:${WHITE_FG}${WIDTH}${RESET} REGION:${WHITE_FG}${REGION}${RESET}"
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: WIDTH_CENTER:${WHITE_FG}${WIDTH_CENTER}${RESET} REGION_CENTER:${WHITE_FG}${REGION_CENTER}${RESET}"
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: CENTERS AFTER ROUNDING - WIDTH_CENTER:${WHITE_FG}${WIDTH_CENTER}${RESET} REGION_CENTER:${WHITE_FG}${REGION_CENTER}${RESET}"

	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: Return value (${WHITE_FG}${REGION_CENTER}${RESET} - ${WHITE_FG}${WIDTH_CENTER}${RESET}):$(( REGION_CENTER - WIDTH_CENTER ))"
