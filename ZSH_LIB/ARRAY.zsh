...
arr_is_populated () {
	...
	[[ ${_DEBUG} -ge ${_LOW_DBG} ]] && dbg "${functrace[1]} called ${0}:${LINENO}: ARGC:${#@}, ARR:${#ARR}"

	[[ ${#} -eq 0 ]] && echo "${0}: ${RED_FG}requires an argument${RESET} of type <ARRAY> ${#}" >&2

	[[ ${ARR[@]} =~ "^ *$" ]] && RC=1 || RC=0
	[[ ${_DEBUG} -ge ${_MID_DBG} ]] && dbg "${0}: Returning:${RC}"
...
arr_long_elem () {
	...
	[[ ${_DEBUG} -ge ${_MID_DBG} ]] && dbg "${0}: LONGEST ELEMENT:${LONGEST} STR:${STR}"
...
arr_long_elem_len () {
	...
	[[ ${_DEBUG} -ge ${_LOW_DBG} ]] && dbg "${functrace[1]} called ${0}:${LINENO}: LONGEST ELEMENT LEN:${LONGEST}"
...
in_array () {
	...
	[[ ${_DEBUG} -ge ${_LOW_DBG} ]] && dbg "${functrace[1]} called ${0}:${LINENO}: ARGC:${#@}"
	[[ ${_DEBUG} -ge ${_MID_DBG} ]] && dbg "${0}: ELEMENT:${ELEMENT}"
	[[ ${_DEBUG} -ge ${_MID_DBG} ]] && dbg "${0}: ALIST:${ALIST}"
