...
arr_is_populated () {
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${functrace[1]} called ${0}:${LINENO}: ARGC:${#@}, ARR:${#ARR}"

	[[ ${#} -eq 0 ]] && echo "${0}: ${RED_FG}requires an argument${RESET} of type <ARRAY> ${#}" >&2

	[[ ${ARR[@]} =~ "^ *$" ]] && RC=1 || RC=0
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: Returning:${RC}"
...
arr_long_elem () {
	...
	[[ ${_DEBUG} -ge ${_HIGH_DBG} ]] && dbg "${0}: LONGEST ELEMENT:${LONGEST} STR:${STR}"
