...
list_item () {
	...
	[[ ${_DEBUG} -ge ${_LIST_LIB_DBG} ]] && dbg "${functrace[1]} called ${0}:${LINENO}: ARGC:${#@}"
	[[ ${_DEBUG} -ge ${_LIST_LIB_DBG} ]] && dbg "${0}: ARGS - MODE:${MODE} LINE_ITEM LEN:${#LINE_ITEM} X_POS:${X_POS} Y_POS:${Y_POS} TOP_OFFSET:${_PAGE_DATA[TOP_OFFSET]} _LIST_NDX:${_LIST_NDX}"
...
list_select () {
	...
	list_set_index_range ${_PAGE_DATA[PAGE_RANGE_TOP]} ${_PAGE_DATA[PAGE_RANGE_BOT]}
	...
	while true;do
		...
		_PAGE_DATA[PAGE_RANGE_TOP]=${LIST_RANGE[1]}
		_PAGE_DATA[PAGE_RANGE_BOT]=${LIST_RANGE[2]}
		...
		if [[ ${_ACTIVE_SEARCH} == 'true' && ${_TARGET_PAGE} -eq ${_PAGE_DATA[PAGE]} ]];then
		...
		while true;do
			...
			case ${KEY} in
				1) DIR_KEY=u;((CURSOR_NDX--));_LIST_NDX=$(list_set_index ${DIR_KEY} ${_LIST_NDX} ${_PAGE_DATA[PAGE_RANGE_TOP]} ${_PAGE_DATA[PAGE_RANGE_BOT]} ${_PAGE_DATA[MAX_ITEM]});; # Up Arrow
				2) DIR_KEY=d;((CURSOR_NDX++));_LIST_NDX=$(list_set_index ${DIR_KEY} ${_LIST_NDX} ${_PAGE_DATA[PAGE_RANGE_TOP]} ${_PAGE_DATA[PAGE_RANGE_BOT]} ${_PAGE_DATA[MAX_ITEM]});; # Down Arrow
				...
				47|60|62)	[[ ${KEY} -eq 47 ]] && MODE=new; # Forward slash
								...
								[[ ${?} -ne 0 ]] && break
								if [[ ${_TARGET_PAGE} -eq ${_PAGE_DATA[PAGE]} ]];then # Same page - move cursor
				...
				a) [[ ${_SELECTABLE} == 'true' ]] && list_toggle_all ${_PAGE_DATA[PAGE_RANGE_TOP]} ${_PAGE_DATA[TOP_OFFSET]} ${_MAX_DISPLAY_ROWS} ${_PAGE_DATA[MAX_ITEM]} ${_PAGE_DATA[PAGE]} ${_PAGE_DATA[MAX_PAGE]} toggle;; # 'a' Toggle all
				...
				0) SELECTED_COUNT=$(list_get_selected_count); # Enter
					...
					if [[ ${SELECTED_COUNT} -eq 0 ]];then
						...
						if [[ ${_CLIENT_WARN} == 'true' ]];then
							list_warn_invisible_rows ${_MAX_DISPLAY_ROWS} ${_PAGE_DATA[PAGE]}
							break 2
						else
